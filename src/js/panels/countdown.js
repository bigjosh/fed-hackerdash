...
(() => {
  ...
  HD.panel('countdown', (ctx) => {
    ...
    function relayout() {
      ...
      o.pad = w < 330 ? 6 : 9;
      o.topY = compact ? 3 : 5;
      o.topH = compact ? 11 : 13;
      o.gapTop = compact ? 5 : 8;
      o.barH = compact ? 5 : 7;
      o.gapBar = compact ? 6 : 9;
      o.tickLbl = h >= 125 ? 12 : 0;
      o.botPad = compact ? 4 : 5;
      o.seq = glyphSeq(hoursMode);
      const plan = seqPlan(o.seq);
      const innerW = w - o.pad * 2;
      // "T−" prefix: font scales with the digits, so budget it as ~0.3 H plus a fixed gap
      const tmU = 0.3;
      const tmGap = 7;
      const rowH = h >= 120 ? 13 : 0;
      const areaTop = o.topY + o.topH + o.gapTop;
      const availFor = (extra) => h - o.botPad - extra - o.tickLbl - o.barH - o.gapBar - areaTop;
      const sideW = U.clamp(Math.round(w * 0.19), 80, 150);
      ...
      if (o.side) {
        ...
        const rh = compact ? 13 : 15;
        const n = U.clamp(Math.floor((o.H + 4) / rh), 1, want.length);
        const keep = want.slice(0, n);
        const shown = order.filter((k) => keep.includes(k));
        const step = shown.length > 1 ? (o.H + 2 - rh) / (shown.length - 1) : 0;
        shown.forEach((k, i) => o.rows.push({ k, y: Math.round(o.digTop - 1 + i * Math.min(step, rh + 6)), h: rh }));
      ...
      o.rSmall = { x: sx0, y: o.digTop + o.H - o.Hs - padS, w: Math.min(w, o.blockR + padS + 2) - sx0, h: o.Hs + padS * 2 };
      ...
      o.rLamp = { x: o.pad - 1, y: o.topY + o.topH / 2 - 4, w: 8, h: 8 };
      const sbh = compact ? 3 : 4;
      o.rTop = { x: 0, y: 0, w, h: sbh };
      o.rBot = { x: 0, y: h - sbh, w, h: sbh };
      scan.style.cssText = `left:${o.blockX - 5}px;top:${o.digTop - 5}px;width:${Math.round(o.blockR - o.blockX + 12)}px;height:${o.H + 10}px`;
    ...
    function buildBase() {
      ...
      const hz = g.createRadialGradient(
        (o.blockX + o.blockR) / 2, o.digTop + o.H / 2, 0,
        (o.blockX + o.blockR) / 2, o.digTop + o.H / 2, Math.max(60, (o.blockR - o.blockX) * 0.7)
      );
      ...
      const lblPx = o.compact ? 9 : 10;
      const lblW = txt(g, 'GRID WIPE IN', o.pad + 10, ty, FUI(lblPx), acc, 'left', o.compact ? 1.4 : 2);
      const subFont = FUI(o.compact ? 9 : 9.5, 600);
      const subLs = o.compact ? 0.8 : 1.4;
      const subX0 = o.pad + 10 + lblW + 14;
      ...
      for (let x = o.pad; x < w - o.pad; x += 24) g.fillRect(x, hy - 1, 1, 3);
      g.fillRect(o.pad, hy - 1, 18, 2);
      ...
      brackets(g, bx + 0.5, by + 0.5, bw - 1, bh - 1, 6, HD.rgba(acc, 0.85), 1.5);
      ...
      o.seq.forEach((k, i) => {
        const x = o.gx[i];
        if (k === 'd') glyphPath(g, 8, x, o.digTop, o.H);
        else if (k === 's') glyphPath(g, 8, x, o.digTop + o.H - o.Hs, o.Hs);
        else glyphPath(g, k === ':' ? COLON : DOT, x, o.digTop, o.H);
      });
      g.fill();
      ...
      const capY = o.digTop + (o.H - o.Hs) / 2 - 1;
      if (o.H - o.Hs >= 18) {
        txt(g, '1/100 S', csX + 1, capY, FUI(9), HD.rgba(C.text, 0.7), 'left', 1);
      }
      if (o.dial) {
        const { x, y, r } = o.dial;
      ...
      if (o.side) {
        ...
        for (const r of o.rows) {
          ...
          txt(g, label, o.sideX, cy, lf, C.dim, 'left', 1.2);
      ...
      if (o.gapBar >= 7) {
        ...
        for (let i = 0; i <= 20; i++) {
          ...
          const big = i % 5 === 0;
          g.fillRect(x, o.barY - (big ? 5 : 3), 1, big ? 4 : 2);
        }
      }
      o.markers = [];
    ...
    function drawHud(pct) {
      ...
      const o = L;
      const acc = accent();
      const vf = FMONO(o.compact ? 9.5 : 10.5, 500);
      const vals = {
        cycle: U.pad(M.cycle),
        abort: '×' + aborts,
        ...
        for (const r of o.rows) {
          ...
          if (r.k === 'ladder') {
            ...
            for (let i = 0; i < 5; i++) {
              ...
              g.fillRect(x, cy - 3, cw, 6);
            }
            o.ladderCell = { x: right - (5 - pi) * (cw + 2) + 2, y: cy - 3, w: cw, h: 6 };
          } else if (r.k === 'chip') {
      ...
      if (o.readRow) {
        ...
        const chipW = 66;
        drawChip(g, w - o.pad - chipW, y - 6, chipW, 12);
        let x = o.pad;
        ...
        for (const [k, v, col] of items) {
          const need = measure(FUI(9), k, 1.2) + 4 + measure(vf, v) + 12;
          if (x + need > w - o.pad - chipW - 6) break;
          x += txt(g, k, x, y, FUI(9), C.dim, 'left', 1.2) + 4;
          x += txt(g, v, x, y, vf, col) + 12;
        }
      }
      if (o.tickLbl) {
        const ly = o.barY + o.barH + 7;
        txt(g, (pct / 10).toFixed(1).padStart(5, '0') + '%', o.pctX, ly, FMONO(9.5, 500), acc, 'left');
      }
    }
    ...
    function drawChip(g, x, y, cw, ch) {
      ...
      g.closePath();
      g.fill();
      txt(g, 'OVERRIDE', x + cw / 2 + 5, cy + 0.5, FUI(9, 700), hot ? '#fff' : C.threat, 'center', 1.4);
    }
    ...
    function paintFx(g, f) {
      ...
      if (f.show) {
        ...
        for (let i = 0; i < o.seq.length; i++) {
          ...
          if (k === 'd' || k === 's') {
            ...
            if (k === 'd') blit(g, big, v, x, y);
            else blit(g, sm, v, x, y + o.H - o.Hs);
          } else if (k === ':') {
            if (f.colon) blit(g, big, COLON, x, y);
          } else {
            blit(g, big, DOT, x, y);
          }
        }
      ...
      g.fillRect(o.pad, o.topY + o.topH / 2 - 3, 6, 6);
    ...
    function drawFloaters(g, now) {
      for (let i = floaters.length - 1; i >= 0; i--) {
        ...
        const px = U.clamp(Math.round(L.H * 0.3), 12, 34);
        const y = L.digTop + L.H * 0.42 - U.ease.outCubic(k) * Math.max(24, L.H * 0.55);
        const x = (L.gx[L.gx.length - 3] + L.gx[L.gx.length - 1]) / 2 + 4;
    ...
    function drawBand(g, now) {
      ...
      const o = L;
      const bh = U.clamp(Math.round(o.H * 0.46), 22, 64);
      const x0 = o.blockX - 5;
      const bw = o.blockR - o.blockX + 12;
      const y0 = Math.round(o.digTop + o.H / 2 - bh / 2);
      ...
      const big = kind === 'deny' ? 'ACCESS DENIED' : 'PAYLOAD RE-ARMED';
      const small = kind === 'deny' ? `BIOMETRIC MISMATCH · ABORT ×${aborts}` : `NEW SIGNAL · CYCLE ${U.pad(M.cycle)}`;
      let px = U.clamp(Math.round(bh * 0.36), 10, 24);
      g.font = FDISP(px);
      g.letterSpacing = '2px';
      ...
      g.textBaseline = 'middle';
      if (!RM) txt(g, big, x0 + bw / 2 + 1.5, cy + 1, FDISP(px), HD.rgba(kind === 'deny' ? C.neon : C.holo2, 0.6), 'center', 2);
      txt(g, big, x0 + bw / 2, cy, FDISP(px), kind === 'deny' ? '#fff' : C.ice, 'center', 2);
      if (twoLine) txt(g, small, x0 + bw / 2, y0 + bh * 0.76, FUI(9, 600), kind === 'deny' ? C.amber : C.holo, 'center', 1.6);
      g.globalAlpha = 1;
    }
